    ...
    def extract_intent(self, message: str, history: list[ChatMessage]) -> WeatherIntent:
        system = SystemMessage(
            content=(
                ...
                "明天、后天、多日预报可用 date=forecast，并尽量填写 target_date 的 ISO 日期；‘最近天气’、‘近期天气’、‘近几天’、‘这几天’以及未指定日期的‘天气怎么样/天气情况如何’都表示今天加未来3天趋势，必须使用 date=forecast 且 days=4，不要误填 current。只有用户明确说‘今天/今日’才使用 today，明确说‘现在/当前/实时’才使用 current。长期气候等当前不支持。"
    ...
    def generate_answer(self, message: str, history: list[ChatMessage], data: WeatherData) -> str:
        system = SystemMessage(
            content=(
                ...
                "不得补充或猜测数据中没有的信息。若数据只含 precipitation_mm，说明为预计降水量，不要称为降雨概率。"
                "当用户询问最近、近期、近几天或这几天天气且数据为多日预报时，按“地点近期天气”“今日”和“未来三天”组织回答：先概括升降温、降水或温差趋势，再逐日列出日期、天气、最低～最高温；风或降水字段存在时一并说明，最后给出仅基于这些数据的穿衣或出行提醒。"
                "若用户询问景点旅游，请给出是否适合、推荐时段、携带物品和风险提醒。"
                "回答简洁、自然，并明确地点和日期。"
