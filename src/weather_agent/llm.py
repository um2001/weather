    ...
    def extract_intent(self, message: str, history: list[ChatMessage]) -> WeatherIntent:
        system = SystemMessage(
            content=(
                ...
                "明天、后天、多日预报可用 date=forecast，并尽量填写 target_date 的 ISO 日期；‘最近天气’、‘近几天’、‘这几天’默认表示未来约3天趋势，必须使用 date=forecast，不要误填 current。长期气候等当前不支持。"
    ...
    def generate_answer(self, message: str, history: list[ChatMessage], data: WeatherData) -> str:
        system = SystemMessage(
            content=(
                ...
                "不得补充或猜测数据中没有的信息。若数据只含 precipitation_mm，说明为预计降水量，不要称为降雨概率。若用户询问景点旅游，请给出是否适合、推荐时段、携带物品和风险提醒。"
                "回答简洁、自然，并明确地点和日期。"
