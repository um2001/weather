...
- 配置大模型后，非天气问题也可进行普通中文对话；天气和旅游问题仍会先查询实时天气数据。
...
```text
用户问题 -> 意图识别
         ├─ weather/travel -> 地点解析 -> 和风天气查询/缓存
         │                  -> 大模型生成建议（失败时回退为标准化回答）
         └─ other          -> 大模型普通聊天回答
         -> 保存会话并返回（天气问题附带结构化天气卡片）
```
...
`status` 为 `success`、`clarification`（需补充地点）、`unsupported` 或 `error`。普通聊天成功时只返回 `reply`，不包含 `weather`。天气服务失败时不会伪造温度等数值。
...
**模型不可用**：天气回答生成失败会回退为规则回答；普通聊天生成失败会返回错误；未配置模型时基础天气查询仍可用，但普通聊天不可用。
